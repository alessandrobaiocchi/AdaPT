...
def train(args, io):
    ...
    model = Pct(args, adaptive=True, layers_to_drop=args.layers_to_drop).to(device)
    print(str(model))
    ...
    for epoch in range(args.epochs):
        ...
        model.train()
        train_pred = []
        drops = np.zeros(len(args.layers_to_drop))
        train_true = []
        idx = 0
        total_time = 0.0
        for data, label in (train_loader):
            ...
            logits, masks = model(data)
            loss = criterion(logits, label)
            for i, m in enumerate(masks):
                m_mean = 1-m.mean(dim=1, keepdims=True)
                loss += args.alpha * ((m_mean - args.drop_ratio[i])**2).mean() / len(masks)
            loss.backward()
            opt.step()
            end_time = time.time()
            ...
            count += batch_size
            drops += np.array([1-m.mean().detach().item() for m in masks])*batch_size
        ...
        wandb.log({"train/loss": train_loss*1.0/count, 
                    "train/acc": metrics.accuracy_score(train_true, train_pred),
                    "epoch": epoch, 
                    "train/perclassacc":metrics.balanced_accuracy_score(train_true, train_pred)
                    }, commit=False)
        for i, l in enumerate(args.layers_to_drop):
            wandb.log({f"train/drop{l}": drops[i]*1.0/count}, commit=False)
        ...
        wandb.log({"val/loss": test_loss*1.0/count, "val/acc": test_acc,"epoch": epoch, "val/perclassacc": avg_per_class_acc})
...
def test(args, io, layers_to_drop=[]):
    test_loader = DataLoader(ModelNet40(partition='test', num_points=args.num_points),
                            batch_size=args.test_batch_size, shuffle=True, drop_last=False)

    device = torch.device("cuda" if args.cuda else "cpu")

    model = Pct(args, adaptive=True, layers_to_drop=layers_to_drop).to(device)
    model = nn.DataParallel(model) 
